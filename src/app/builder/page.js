...
// ΓöÇΓöÇΓöÇ COLOUR PALETTE DEFINITIONS ΓöÇΓöÇΓöÇ
const PALETTE = {
...
// ΓöÇΓöÇΓöÇ STYLE PRESETS ΓöÇΓöÇΓöÇ
const PRESETS = {
...
// ΓöÇΓöÇΓöÇ OFFICE DESIGNER PROCEDURAL TEXTURES ΓöÇΓöÇΓöÇ
...
export default function BuilderPage() {
  ...
  // ΓöÇΓöÇΓöÇ React states mirroring active options ΓöÇΓöÇΓöÇ
  ...
  const [hudText, setHudText] = useState("Γ£ª Click a part to customise");
  const [showHud, setShowHud] = useState(false);

  // ΓöÇΓöÇΓöÇ Production & Export States ΓöÇΓöÇΓöÇ
  const [productionModalOpen, setProductionModalOpen] = useState(false);
  const [currentProductionSpec, setCurrentProductionSpec] = useState(null);

  // ΓöÇΓöÇΓöÇ Mobile Responsive Toggle States ΓöÇΓöÇΓöÇ
  const [mobileLeftOpen, setMobileLeftOpen] = useState(false);
  const [mobileRightOpen, setMobileRightOpen] = useState(false);

  // ΓöÇΓöÇΓöÇ Kitchen Designer States ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ Office Designer States (Rebuilt from Reference HTML) ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ TV Wall Unit States ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ Cabinet States ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ Bed States ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ Shelving Unit States ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ Dressing Table States ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ TV Wall Extra States ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ Cabinet Extra States ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ Bed Extra States ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ Shelves Extra States ΓöÇΓöÇΓöÇ
...
const buildWardrobe = useCallback(() => {
    ...
    // ΓöÇΓöÇΓöÇ DOORS (metallic frames + glass/mirror panel support) ΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇΓöÇ INTERIOR SHELVES (always visible, split with LED channel) ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ 3D KITCHEN MESH GENERATOR ΓöÇΓöÇΓöÇ
  const buildKitchen = useCallback(() => {
    ...
    // Backsplash (always visible ΓÇö it sits directly behind the countertop)
    ...
    // Room Walls ΓÇö only rendered when showWalls is enabled
    if (showWalls) {
      // Back Wall
...
tallCabinetType, tallCabinetsCount, countertopMaterial, countertopThickness, countertopWaterfall, countertopColor,
  ...
  // ΓöÇΓöÇΓöÇ 3D OFFICE MESH GENERATOR (Rebuilt from Reference HTML) ΓöÇΓöÇΓöÇ
  const buildOffice = useCallback(() => {
    ...
    // ΓöÇΓöÇΓöÇ ENVIRONMENT ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    // Floor
    ...
    // ΓöÇΓöÇΓöÇ DESK ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓÇö Right side drawers if 'both' or 'right'
    ...
    // ΓöÇΓöÇΓöÇ WALL CABINET ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓÇö Right tall side unit
    cabGrp.add(bx(sideW, officeCabH, officeCabD, _cabMat, officeCabW / 2 - sideW / 2, officeCabH / 2, 0));
    cabGrp.add(bx(sideW - .02, officeCabH - .03, .02, _cabMat, officeCabW / 2 - sideW / 2, officeCabH / 2, officeCabD / 2 - .009));

    // ΓÇö Center vertical dividers
    ...
    // ΓÇö Decorative back panels per upper section
    ...
    // ΓÇö Open shelves in upper zone
    ...
    // ΓÇö Lower zone: floor shelf at lH
    cabGrp.add(bx(cenW + .005, shT, officeCabD - .022, _cabMat, 0, lH, 0));
    // ΓÇö Lower side panels
    cabGrp.add(bx(sideW, lH, officeCabD, _cabMat, -officeCabW / 2 + sideW / 2, lH / 2, 0));
    cabGrp.add(bx(sideW, lH, officeCabD, _cabMat, officeCabW / 2 - sideW / 2, lH / 2, 0));

    // ΓÇö Lower door panels ΓÇö pivot-based (children of cabGrp so they inherit its z=-2.9 offset)
    ...
    for (let d = 0; d < nd; d++) {
      ...
      const pivot = new THREE.Group();
      // Position in cabGrp local space ΓÇö front face of cabinet
      pivot.position.set(hingeX, lH / 2 + .01, officeCabD / 2);
      cabGrp.add(pivot);  // child of cabGrp ΓÇö inherits the -2.9 z offset
  ...
  const buildTVWall = useCallback(() => {
    ...
    // ΓöÇΓöÇ Wall panel behind TV ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const panelW = W * 1.35;
    const panelH = 2.80;
    ...
    // ΓöÇΓöÇ TV Screen ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ LED Backlight ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if (tvWallLed) {
    ...
    // ΓöÇΓöÇ Sound Bar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if (tvSoundBar) {
    ...
    // ΓöÇΓöÇ Floating Media Console ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const cabH = 0.44;
    ...
    // ΓöÇΓöÇ Floating side shelves ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // Cabinet builder ΓÇö compact carcass with configurable drawer rows, an
  ...
  const buildCabinet = useCallback(() => {
    ...
    // ΓöÇΓöÇ Legs ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if (cabinetLegs !== "none") {
    ...
    // ΓöÇΓöÇ Carcass panels ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const base = legH;
    ...
    // ΓöÇΓöÇ Top display zone (open or closed) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if (cabinetOpenTop) {
    ...
    // ΓöÇΓöÇ Mid shelf (highboy / filing) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Lower door section ΓÇö pivot-based so Open Doors animates them ΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Upper drawer section (highboy / filing only) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if (cabinetStyle !== "sideboard") {
    ...
    // ΓöÇΓöÇ Tabletop reveal edge ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  // Bed builder ΓÇö frame, mattress, pillows, headboard styles, optional
  ...
  const buildBed = useCallback(() => {
    ...
    // ΓöÇΓöÇ Platform / frame ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Mattress ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Duvet / Sheet ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Pillows ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Headboard ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const hbConfigs = {
    ...
    // ΓöÇΓöÇ Footboard ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const fbH = bedHeadboard === "tall" ? 0.32 : 0.22;
    add(mesh(box(W + 0.06, fbH, 0.045), app.M.body, 0, legH + fbH/2, L/2 + 0.022, "footboard"), "body");

    // ΓöÇΓöÇ Under-bed storage drawers ΓÇö slide animation via drawerPivots ΓöÇΓöÇΓöÇΓöÇΓöÇ
    if (bedStorage) {
    ...
    // ΓöÇΓöÇ Under-bed LED ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if (bedLedUnder) {
    ...
    // ΓöÇΓöÇ Nightstands ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Bench at foot of bed ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if (bedBench) {
  ...
  // ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  // Shelving unit builder ΓÇö open frame with a choice of layout styles
  ...
  const buildShelves = useCallback(() => {
    ...
    // ΓöÇΓöÇ Material based on shelfMaterial state ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Frame ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if (isMetalFrame) {
    ...
    // ΓöÇΓöÇ Back panel ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if (shelfBackPanel) {
    ...
    // ΓöÇΓöÇ Shelf boards ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ LED strips under each shelf ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Decorative items ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  // Dressing table builder ΓÇö vanity desk with drawer block, configurable
  ...
  const buildDressingTable = useCallback(() => {
    ...
    // ΓöÇΓöÇ Drawer tower (left side) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const towerW = W * 0.30;
    ...
    // ΓöÇΓöÇ Mirror tower (right side) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const towerR = W * 0.22;
    ...
    // ΓöÇΓöÇ Tabletop ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    add(mesh(box(W + 0.025, T*1.5, D + 0.025), bodyMat, 0, tableH + T*0.75, 0, "dtTop"), "body");

    // ΓöÇΓöÇ Knee-hole legs (slender) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Mirror ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Cosmetics on tabletop ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Matching stool ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if (dressingStool) {
      const cushMat  = new THREE.MeshStandardMaterial({ color: 0xE8D4C4, roughness: 0.88 });
  ...
  useEffect(() => {
    ...
    // Face color (door panels + drawer fronts) ΓÇö independently overrideable
    ...
    if (doorStyle === "glass") {
      ...
      // Solid ΓÇö copy from drawerFront (face color)
  ...
  // LED lighting live update (without rebuild) ΓÇö applies to ALL categories
  useEffect(() => {
    const app = appRef.current;
          ...
          if (hits.length) {
            ...
            if (activeCategory === "office") {
              if (grp === "ext-drawer") {
                ...
                if (dp) {
                  ...
                  triggerHud(dp.open ? "≡ƒôª Drawer opened" : "≡ƒôª Drawer closed");
                }
              }
              ...
              triggerHud(`Γ£ª Selected: ${topObj.name || grp}`);
            ...
            if (grp === "doors" || (grp === "handles" && m.parent && m.parent !== app.root)) {
              ...
              if (dp) {
                ...
                triggerHud(isOpen ? "≡ƒÜ¬ Door closed" : "≡ƒÜ¬ Door opened");
              }
              return;
            }
  ...
  const handleApplyPreset = (presetKey) => {
    ...
    triggerNotification("Γ£ª " + presetKey.charAt(0).toUpperCase() + presetKey.slice(1) + " style applied");
  };
  // Export handlers
...
# Countertop: ${activeCategory === "kitchen" ? countertopMaterial : "N/A"}`;
  ...
  const handleRunAI = async (text) => {
    if (!text.trim()) return;
    triggerNotification("✦ AI is designing...");
    ...
    try {
      ...
      if (data.success && data.parameters) {
        ...
        // 1. Switch Category
        if (params.type) {
          setActiveCategory(params.type);
        }
        
        // 2. Set Dimensions
        if (params.dimensions) {
          if (params.dimensions.width) setWidth(params.dimensions.width);
          if (params.dimensions.height) setHeight(params.dimensions.height);
          if (params.dimensions.depth) setDepth(params.dimensions.depth);
        }
        
        // 3. Set Material/Color
        if (params.material) {
          const mat = params.material.toLowerCase();
          setActiveColor(mat);
          setActiveFaceColor(mat);
          setKitchenCabinetWoodType(mat);
          setKitchenCabinetMatteColor(mat);
        }
        
        // 4. Set Handle Style
        if (params.handleStyle) {
          const hs = params.handleStyle.toLowerCase();
          if (hs.includes("gold")) setHandleStyle("gold");
          else if (hs.includes("silver")) setHandleStyle("silver");
          else if (hs.includes("black")) setHandleStyle("black");
          else if (hs.includes("chrome")) setHandleStyle("chrome");
          else if (hs.includes("hidden") || hs.includes("push")) setHandleStyle("hidden");
        }
        
        // 5. Set Door Type
        if (params.doorType) {
          const dt = params.doorType.toLowerCase();
          if (dt.includes("glass")) setDoorStyle("glass");
          else if (dt.includes("mirror")) setDoorStyle("mirror");
          else if (dt.includes("frosted")) setDoorStyle("frosted");
          else setDoorStyle("solid");
        }
        
        // 6. Set LED
        if (params.ledLighting) {
          setLedLighting(params.ledLighting);
        }
        
        // 7. Extract module stats (sections, drawers, shelves)
        if (params.modules && Array.isArray(params.modules)) {
          setSections(params.modules.length);
          const totalDrawers = params.modules.reduce((sum, m) => sum + (m.drawerRows || 0), 0);
          setExtDrawerRows(Math.min(3, totalDrawers));
        }

        // 8. Handle Kitchen-specific params
        if (params.type === "kitchen") {
          if (params.style) {
            setKitchenCabinetMaterial(params.style === "classic" || params.style === "luxury" ? "wood" : "matte");
          }
        }
        
        triggerNotification(`✦ AI generated ${params.style || "modern"} ${params.type || "design"}`);
      }
    } catch (err) {
      console.error("AI Generation failed:", err);
      triggerNotification("⚠️ AI Generation failed");
    }
  };
  ...
  return (
    <div className="builder-root-container">
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        /* ΓöÇΓöÇΓöÇ MOBILE-ONLY UTILITY ΓöÇΓöÇΓöÇ */
        .mob-only { display: none; }

        /* ΓöÇΓöÇΓöÇ TABLET: max-width 1024px ΓöÇΓöÇΓöÇ */
        ...
        /* ΓöÇΓöÇΓöÇ MOBILE: max-width 768px ΓöÇΓöÇΓöÇ */
        ...
        /* ΓöÇΓöÇΓöÇ SMALL PHONE: max-width 480px ΓöÇΓöÇΓöÇ */
      ...
      <div className="topbar">
        <div className="logo-area">
          <a href="/" className="home-btn">ΓåÉ Home</a>
          <a href="/" className="logo">Furni AI</a>
        </div>
        <div className="file-name">
          <span style={{ color: "var(--accent)" }}>≡ƒôü</span>
          <input defaultValue="Custom Wardrobe ΓÇö 3D" readOnly />
          <span style={{ color: "var(--muted2)" }}>┬╖ Auto-saved</span>
        </div>
        <div className="top-right">
          <button className="mob-btn mob-only" onClick={() => { setMobileLeftOpen(!mobileLeftOpen); setMobileRightOpen(false); }} title="Tools">Γÿ░</button>
          <button className="mob-btn mob-only" onClick={() => { setMobileRightOpen(!mobileRightOpen); setMobileLeftOpen(false); }} title="Properties">ΓÜÖ</button>
          <div className="vtog">
          ...
          <button className="tb gold cursor-pointer" onClick={handleExportPNG}>Export Γåô</button>
        </div>
      </div>
      ...
      <div className="workspace">
        ...
        <div className={`lsb ${mobileLeftOpen ? 'mob-open' : ''}`}>
          <div className="lsb-sec" style={{ paddingTop: "14px" }}>
            ...
            {[
              { id: "wardrobe",       label: "Wardrobe",       emoji: "≡ƒÜ¬", bg: "linear-gradient(135deg,#c8a96e,#9a7040)" },
              { id: "kitchen",        label: "Kitchen",         emoji: "≡ƒì│", bg: "linear-gradient(135deg,#b59beb,#7a5ab0)" },
              { id: "office",         label: "Office",          emoji: "≡ƒÆ╝", bg: "linear-gradient(135deg,#d4a5e8,#9a60c0)" },
              { id: "tv-wall",        label: "TV Wall",         emoji: "≡ƒô║", bg: "linear-gradient(135deg,#9bcbeb,#5090b8)" },
              { id: "cabinet",        label: "Cabinet",         emoji: "≡ƒùä∩╕Å", bg: "linear-gradient(135deg,#c5a5e8,#8058b8)" },
              { id: "bed",            label: "Bed",             emoji: "≡ƒ¢Å∩╕Å", bg: "linear-gradient(135deg,#a5b0e8,#5060b8)" },
              { id: "shelves",        label: "Shelves",         emoji: "≡ƒôÜ", bg: "linear-gradient(135deg,#a5e8b0,#40a858)" },
              { id: "dressing-table", label: "Dressing Table",  emoji: "≡ƒ¬₧", bg: "linear-gradient(135deg,#e8d4a5,#b89050)" },
            ].map((f) => (
              <div
                key={f.id}
          ...
          <button className={`scene-btn ${appRef.current.doorsOpen ? "on" : ""}`} onClick={handleToggleDoors}>
            <span style={{ background: "#a07040", width: "16px", height: "16px", borderRadius: "4px", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: "0.6rem", marginRight: "6px" }}>≡ƒÜ¬</span>
            Open Doors
          </button>
          <button className={`scene-btn ${appRef.current.interiorVisible ? "on" : ""}`} onClick={handleToggleInterior}>
            <span style={{ background: "#c8a96e", width: "16px", height: "16px", borderRadius: "4px", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: "0.6rem", marginRight: "6px" }}>≡ƒôª</span>
            Show Interior
          </button>
          <button className="scene-btn" onClick={handleResetCamera}>
            <span style={{ background: "#a59be8", width: "16px", height: "16px", borderRadius: "4px", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: "0.6rem", marginRight: "6px" }}>≡ƒÄÑ</span>
            Reset Camera
          </button>
          <button className="scene-btn" onClick={() => triggerNotification("Selected all doors")}>
            <span style={{ background: "#5a5855", width: "16px", height: "16px", borderRadius: "4px", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: "0.6rem", marginRight: "6px" }}>∩╝ï</span>
            Select All Doors
          </button>
          ...
          <div style={{ padding: "0 12px 12px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
              ...
              {/* Card 4: plus */}
              <div style={{ aspectRatio: "1", background: "var(--bg3)", border: "1px dashed var(--border2)", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: ".9rem", color: "var(--muted2)", cursor: "pointer" }}>∩╝ï</div>
            </div>
          </div>
        </div>
        ...
        <div className="cw" ref={wrapRef}>
          ...
          <div className="hint">≡ƒû▒ Drag to orbit ┬╖ Scroll to zoom ┬╖ Click any part</div>
          <div className="bbbar">
            <input
              className="bbi"
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="Describe styleΓÇª e.g. Luxury gold wardrobe with LED"
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  handleRunAI(prompt);
                  setPrompt("");
        ...
        <div className={`rp ${mobileRightOpen ? 'mob-open' : ''}`}>
          {activeCategory === "kitchen" ? (
            <>
              ...
              <div className="rps">
                <div className="rpt">Γ£ª AI Prompt</div>
                <div className="aibox">
                  <textarea
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                    placeholder="Describe your kitchen styleΓÇª&#10;e.g. Modern white kitchen with oak cabinets and marble countertop"
                  />
              ...
              <div className="rps" style={{ paddingBottom: "10px" }}>
                ...
                >
                  <span style={{ background: showWalls ? "#6a9bc8" : "#5a5855", width: "16px", height: "16px", borderRadius: "4px", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: "0.6rem", marginRight: "6px" }}>≡ƒÅá</span>
                        ...
                        <div key={m.id} style={{ display: "flex", flexDirection: "column", background: "rgba(0,0,0,0.02)", border: "1px solid var(--border)", borderRadius: "6px", padding: "8px", gap: "4px" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                              <span style={{ fontSize: "14px" }}>{typeEmojis[m.type] || "≡ƒôü"}</span>
              ...
              <div className="rps">
                ...
                <div className="size-grid" style={{ marginBottom: "10px" }}>
                  ...
                  <div className="section-btns" style={{ marginBottom: "8px" }}>
                    {[
                      { id: "wood", label: "≡ƒ¬╡ Wood" },
                      { id: "matte", label: "≡ƒÄ¿ Matte" },
                      { id: "premium", label: "≡ƒÆÄ Premium" },
                    ].map((fin) => (
                      <div
                        key={fin.id}
                ...
                <div className="optlist">
                  {[
                    { id: "gold", label: "ΓÇö Gold Bar" },
                    { id: "silver", label: "ΓùÄ Silver Knob" },
                    { id: "black", label: "Γû¼ Black Strip" },
                    { id: "hidden", label: "Γèÿ Hidden Push" },
                    { id: "chrome", label: "Γ£ª Chrome" },
                  ].map((hStyle) => (
                    <div
                      key={hStyle.id}
                ...
                <div className="size-grid">
                  <div className="size-row" style={{ marginBottom: "6px" }}>
                    ...
                    <div className="optlist" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px" }}>
                      {[
                        { id: "marble", label: "≡ƒÆÄ Marble" },
                        { id: "quartz", label: "Γ£¿ Quartz" },
                        { id: "granite", label: "≡ƒ¬¿ Granite" },
                        { id: "concrete", label: "≡ƒº▒ Concrete" },
                        { id: "wood", label: "≡ƒ¬╡ Wood" },
                        { id: "ceramic", label: "≡ƒÅ║ Ceramic" },
                      ].map((mat) => (
                        <div
                          key={mat.id}
                    ...
                    <>
                      <div className="size-row" style={{ marginBottom: "8px" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                          <button className="sec-n cursor-pointer" onClick={() => setOfficeDeskDrawerCount(Math.max(1, officeDeskDrawerCount - 1))} style={{ padding: "4px 10px" }}>ΓêÆ</button>
              ...
              <div className="rps">
                ...
                <div className="size-grid">
                  <div className="size-row" style={{ marginBottom: "8px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      <button className="sec-n cursor-pointer" onClick={() => setOfficeCabOpenShelves(Math.max(1, officeCabOpenShelves - 1))} style={{ padding: "4px 10px" }}>ΓêÆ</button>
                  ...
                  <div className="size-row" style={{ marginBottom: "8px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      <button className="sec-n cursor-pointer" onClick={() => setOfficeCabLowerDoors(Math.max(2, officeCabLowerDoors - 1))} style={{ padding: "4px 10px" }}>ΓêÆ</button>
            ...
            <>
              ...
              {activeCategory === "tv-wall" && (
                <div className="rps">
                  ...
                  >
                    {tvWallLed ? "Γ£ô" : "Γùï"} LED Backlight Glow
                  </button>
                  <button
                  ...
                  >
                    {tvWallStorage ? "Γ£ô" : "Γùï"} Enclosed Storage Cabinet
                  </button>
                  <div className="size-row" style={{ marginBottom: "8px" }}>
                  ...
                  >
                    {tvConsoleLegs ? "Γ£ô" : "Γùï"} Hairpin Console Legs
                  </button>
                </div>
              )}
              ...
              {activeCategory === "cabinet" && (
                <div className="rps">
                  ...
                  >
                    {cabinetOpenTop ? "Γ£ô" : "Γùï"} Open-Top Display Compartment
                  </button>
                  <div className="size-row" style={{ marginBottom: "8px" }}>
                  ...
                  >
                    {cabinetGlassDoors ? "Γ£ô" : "Γùï"} Glass Doors
                  </button>
                </div>
              )}
              ...
              {activeCategory === "bed" && (
                <div className="rps">
                  ...
                  >
                    {bedStorage ? "Γ£ô" : "Γùï"} Under-Bed Storage Drawers
                  </button>
                  <button
                  ...
                  >
                    {bedLedUnder ? "Γ£ô" : "Γùï"} Under-Bed LED Glow
                  </button>
                  <div className="size-row" style={{ marginBottom: "8px" }}>
                  ...
                  >
                    {bedBench ? "Γ£ô" : "Γùï"} Foot-of-Bed Bench
                  </button>
                </div>
              )}
              ...
              {activeCategory === "shelves" && (
                <div className="rps">
                  ...
                  >
                    {shelfBackPanel ? "Γ£ô" : "Γùï"} Back Panel
                  </button>
                  <div className="size-row" style={{ marginBottom: "8px" }}>
                  ...
                  >
                    {shelfLighting ? "Γ£ô" : "Γùï"} LED Shelf Lighting
                  </button>
                  <button
                  ...
                  >
                    {shelfDecorItems ? "Γ£ô" : "Γùï"} Books & Decor Items
                  </button>
                </div>
              )}
              ...
              {activeCategory === "dressing-table" && (
                <div className="rps">
                  ...
                  >
                    {dressingStool ? "Γ£ô" : "Γùï"} Matching Stool
                  </button>
                </div>
              )}
            ...
            <>
              ...
              <div className="rps">
                ...
                <div className="optlist">
                  {[
                    { id: "solid", label: "≡ƒ¬╡ Solid Panel" },
                    { id: "glass", label: "≡ƒö╖ Glass Panel" },
                    { id: "mirror", label: "≡ƒ¬₧ Full Mirror" },
                    { id: "frosted", label: "Γ¥ä∩╕Å Frosted Glass" },
                  ].map((style) => (
                    <div
                      key={style.id}
              ...
              <div className="rps">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    triggerNotification(!hangerRods ? "Hanger rods added ΓÇö top shelf removed for space" : "Hanger rods removed");
                  }}
                >
                  <span style={{ background: hangerRods ? "#8a7adc" : "#5a5855", width: "18px", height: "18px", borderRadius: "4px", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: "0.6rem", marginRight: "8px" }}>≡ƒ¬¥</span>
                ...
                >
                  <span style={{ background: "#f4a261", width: "18px", height: "18px", borderRadius: "4px", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: "0.6rem", marginRight: "8px" }}>≡ƒôé</span>
                  Open All Drawers
                </button>
                
                <p style={{ fontSize: ".68rem", color: "var(--muted2)", marginTop: "7px" }}>
                  ≡ƒÆí Click any drawer to open/close it individually
                </p>
              </div>
